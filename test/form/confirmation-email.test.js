...
describe('Confirmation email', () => {
  ...
  beforeAll(async () => {
    server = await createServer({
      formFileName: 'confirmation-email.js',
  ...
  describe('Page: /summary', () => {
    ...
    it('should render confirmation email field', () => {
      const $email = container.getByLabelText('Confirmation email (optional)')
    ...
    it('should display error in invalid email address', async () => {
      const { container, response } = await renderResponse(server, {
        url: `${basePath}/summary`,
        method: 'POST',
        headers,
        payload: {
          ...
          userConfirmationEmailAddress: 'bad@bad-email'
        }
      })
    ...
    it('should redirect to the complete page on submit, with valid email', async () => {
      ...
      const response = await server.inject({
        ...
        payload: {
          crumb: csrfToken,
          action: FormAction.Send,
          userConfirmationEmailAddress: 'valid@valid-email.com'
        }
      })
