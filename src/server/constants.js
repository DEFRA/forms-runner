export const PREVIEW_PATH_PREFIX = '/preview'
export const ERROR_PREVIEW_PATH_PREFIX = '/error-preview'
export const FORM_PREFIX = '/form'
export const HOMEPAGE_PREFIX = '/homepage'
export const SIGN_IN_PATH = '/auth/sign-in'
export const RESUME_FORM_PATH = '/resume-form'
export const RESUME_VERIFY_PATH = '/resume-form-verify'
export const RESUME_ERROR_PATH = '/resume-form-error'
export const RESUME_SUCCESS_PATH = '/resume-form-success'
export const SIGN_OUT_PATH = '/auth/sign-out'
export const SIGNED_OUT_PATH = '/auth/signed-out'
export const CALLBACK_PATH = '/auth/callback'
export const SAVE_AND_EXIT_PAYLOAD = 'SAVE_AND_EXIT_PAYLOAD'
export const EN_GB = 'en-GB'
export const CY = 'cy'
