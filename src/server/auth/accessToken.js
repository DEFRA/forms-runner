...
/**
 * Uses the refresh token to get a new access token. It does not read or write
 * the session; the citizen-session scheme saves the result. Two requests can
 * refresh at the same time; the refresh token is not rotated, so both succeed.
 * @param {RequestContext} request
 * @param {string} sub - the citizen the tokens were issued for. A refreshed
 *   ID token must name the same one.
 * @param {TokenSet} tokenSet
 * @returns {Promise<TokenSet | undefined>} the new tokens, or undefined when
 *   the provider could not refresh them for now, so a later request can try
 *   again.
...
export async function refreshAccessToken(request, sub, tokenSet) {
  ...
  if (claims && claims.sub !== sub) {
    ...
    throw new SignInRequiredError('subjectMismatch')
  }
